...
$(document).ready(function() {
	...
	/* Interior Blog Secondary Stories Heights */
	if($('.gsv-moving-ideas-sub-blog-secondary-stories')){
		var othernews = $('.gsv-moving-ideas-sub-blog-secondary-stories-other-news').height();
		var upcomingdates = $('.gsv-moving-ideas-sub-blog-secondary-stories-upcoming-dates').height();
		
		if(othernews > upcomingdates){
			$('.gsv-moving-ideas-sub-blog-secondary-stories-upcoming-dates').height(othernews)
		}else{
			$('.gsv-moving-ideas-sub-blog-secondary-stories-other-news').height(upcomingdates);
		}
	}
	
	
	/* Interior Blog Top Picks Heights */
	if($('.gsv-moving-ideas-sub-blog-top-picks-list')){
		var story1 = $('.gsv-moving-ideas-sub-blog-top-picks-story').eq(0).height();
		var story2 = $('.gsv-moving-ideas-sub-blog-top-picks-story').eq(1).height();
		var story3 = $('.gsv-moving-ideas-sub-blog-top-picks-story').eq(2).height();
		
		
		if(story1 > story2){
			if(story1 > story3){
				$('.gsv-moving-ideas-sub-blog-top-picks-story').eq(1).height(story1)
				$('.gsv-moving-ideas-sub-blog-top-picks-story').eq(2).height(story1)
			}else{
				$('.gsv-moving-ideas-sub-blog-top-picks-story').eq(0).height(story3)
				$('.gsv-moving-ideas-sub-blog-top-picks-story').eq(1).height(story3)
			}
		}else{
			if(story2 > story3){
				
				$('.gsv-moving-ideas-sub-blog-top-picks-story').eq(0).height(story2)
				$('.gsv-moving-ideas-sub-blog-top-picks-story').eq(2).height(story2)
			}else{
				$('.gsv-moving-ideas-sub-blog-top-picks-story').eq(0).height(story3)
				$('.gsv-moving-ideas-sub-blog-top-picks-story').eq(1).height(story3)
			}
	...
	if(error_list == ''){
		...
		$.ajax({
			...
			success: function(msg) {
				...
				if($('[data-js="gsv-newsletter-signup-form"]').length > 0){
