...
$subsection_without_content =
array(
	"sub_section_title_only" => array(
	"name" => "sub_section_title_only",
	"std" => "",
...
function build_title_without_content($title){
	global $subsection_without_content, $post;
	...
	<?php
	foreach($subsection_without_content as $meta_box) {
		...
		if($meta_box['name'] == 'sub_section_title_only'){
...
function save_subsection_without(){ 
		
	global $subsection_without_content, $post, $_POST, $post_id;
	save_component($subsection_without_content, $post, $_POST, $post_id, plugin_basename(__FILE__));
}

/** Hook actions into WP **/
add_action('save_post', 'save_subsection_without');
