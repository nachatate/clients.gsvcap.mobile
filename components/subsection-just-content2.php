...
$subsection_just_content2 =
array(
	"sub_section_content2" => array(
	"name" => "sub_section_content2",
	"std" => "",
...
function build_title_just_content2($title){
	global $subsection_just_content2, $post;
	?>
	
	
	
	<?php
	foreach($subsection_just_content2 as $meta_box) {
	...
	echo'<div class="cp-product-attribute-section cp-product-attribute-section-price"><span class="cp-product-attribute-section-title"><input type="hidden" name="'.$meta_box['name'].'_noncename" id="'.$meta_box['name'].'_noncename" value="'.wp_create_nonce( plugin_basename(__FILE__) ).'" />'.$title.'</span>';
		...
		}else if($meta_box['name'] == 'sub_section_content2'){ 
...
function save_subsection_just_content2(){ 
		
	global $subsection_just_content2, $post, $_POST, $post_id;
	save_component($subsection_just_content2, $post, $_POST, $post_id, plugin_basename(__FILE__));
}

/** Hook actions into WP **/
add_action('save_post', 'save_subsection_just_content2');
